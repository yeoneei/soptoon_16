...
res.status(200).send(utils.successFalse(statusCode.BAD_REQUEST),responseMessage.NULL_VALUE);
    ...
    const hashedPassword = await crypto.pbkdf2(userInfo.password, salt.toString('base64'), 1000, 32, 'SHA512');
    userInfo.salt=hashedPassword; //db에 salt 저장하기
    userInfo.hashedPassword = userInfo.salt.toString('base64');
    ...
    const insertQuery = 'INSERT INTO user(id,name,nickname,salt,hashedPassword) VALUES(?,?,?,?,?)';
    const insertResult = await db.queryParam_Parse(insertQuery, [userInfo.id, userInfo.name, userInfo.nickname,userInfo.salt,userInfo.hashedPassword]);
